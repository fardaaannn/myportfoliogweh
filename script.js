...
function setBodyPadding() {
  ...
  if (nav) {
    ...
    document.body.style.paddingTop =
      navHeight + CONFIG.NAV_PADDING_OFFSET + "px";
  }
}
  ...
  if (form) {
    form.addEventListener("submit", (e) => {
      ...
      if (isValid) {
        ...
        const waMessage =
          `*Pesan dari Website Fardan.Dev*%0A%0A` +
          `*Nama:* ${nama}%0A%0A` +
          `*Pesan:*%0A${pesan}`;
...
function initCustomCursor() {
  ...
  const hoverElements = document.querySelectorAll(
    "a, button, .kartu-ios, .project-card, .social-link, input, textarea, .theme-switch",
  );
  ...
  document.addEventListener("mouseenter", () => {
    ...
    trails.forEach(
      (trail, i) => (trail.element.style.opacity = (1 - i / trailCount) * 0.5),
    );
  });
}
...
function initChatbot() {
  ...
  function getPageContext() {
    ...
    if (fileName.includes("skills") || fileName.includes("skill"))
      return "Skills";
    if (fileName.includes("project")) return "Projects";
    if (fileName.includes("kontak") || fileName.includes("contact"))
      return "Kontak";
    return "Home";
  }
...
🌍 MULTILINGUAL: If someone asks in English, reply in English but keep the jutek energy. Match the language but KEEP THE VIBES.`;
  ...
  function getGreeting(personality) {
    ...
    const greetings = {
      jutek: {
        Profil:
          "Oh, lo lagi di halaman Profil. Mau kepo soal Fardan? Ya udah tanya aja, gue jawab kalo mood.",
        Skills:
          "Lo lagi di halaman Skills. Mau tau skill Fardan? Tanya aja, tapi jangan nanya yang aneh-aneh.",
        Projects:
          "Lo lagi ngecek Projects. Mau tanya soal project? Silakan, gue lagi baik hari ini.",
        Kontak:
          "Lo di halaman Kontak. Mau hubungin Fardan? Ya hubungin aja langsung, ngapain nanya gue.",
    ...
    return (
      greetings[personality]?.[currentPage] || greetings[personality]?.Home
    );
  }
  ...
  async function sendMessage() {
    ...
    try {
      ...
      addMessage(
        "Gagal terhubung ke server. Pastikan koneksi internet kamu lancar ya! 🌐",
        "bot",
      );
      conversationHistory.pop();
    }
...
const musicPlayer = {
  ...
  toggleLoop() {
    ...
    if (this.btnLoop) {
      ...
      this.btnLoop.style.backgroundColor = this.isLooping
        ? "var(--accent)"
        : "";
